...
// This whole thing is taken from https://github.com/nemutas/azulejo/, give credit
void main() {
  vec2 uv = vUv;
  ...
  for(; i < 6.0; i++) {
    ...
    if (2.0 <= i) {
      ...
      px *= 0.5;
      vec3 c1 = texture(uMap, iuv + px * vec2(0, 0) + px * 0.5).rgb;
      vec3 c2 = texture(uMap, iuv + px * vec2(1, 0) + px * 0.5).rgb;
      vec3 c3 = texture(uMap, iuv + px * vec2(0, 1) + px * 0.5).rgb;
      vec3 c4 = texture(uMap, iuv + px * vec2(1, 1) + px * 0.5).rgb;
  ...
  vec3 pattern = texture(uTileMap,  mapOffset + ruv * uMapPx).rgb;
  ...
  // gl_FragColor = vec4(sat(pattern + b), 1.0);
  gl_FragColor = vec4(sat(pattern), 1.0);
